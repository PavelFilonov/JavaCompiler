...
def make_parser():
    ...
    method = type_ + ident + LPAR + params + RPAR + LBRACE + stmt_list + RBRACE
    ...
    class_ = pp.Keyword('class').suppress() + ident + LBRACE + stmt_list.ignore(pp.javaStyleComment).ignore(
        pp.dblSlashComment) + RBRACE
    classes = pp.ZeroOrMore(class_)
            ...
            with suppress(NameError):
                ...
                if not inspect.isabstract(cls):
                    def parse_action(s, loc, tocs):
                        if cls is MethodNode:
                            return MethodNode(tocs[0], tocs[1], tocs[2:-1], tocs[-1], loc=loc)
                        else:
